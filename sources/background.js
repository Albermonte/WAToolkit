...
function onMessage(messageEvent, sender, callback) {
    if (messageEvent.name == "getIsBackgroundPage") {
        ...
        if (messageEvent.warn) {
            chrome.action.setIcon({ path: { "19": "img/favicon19warn.png", "38": "img/favicon38warn.png" } });
        }
        else {
            chrome.action.setIcon({ path: { "19": "img/favicon19.png", "38": "img/favicon38.png" } });
        }
        if (messageEvent.badgeText != undefined) {
            chrome.action.setBadgeText({ text: messageEvent.badgeText });
        }
        if (messageEvent.tooltipText != undefined) {
            chrome.action.setTitle({ title: messageEvent.tooltipText });
        }
    }
